...
# @keyword manip
print.reshape_matrix <- function(x, digits=getOption("digits"), ...) {
...
# @argument Argument required to match generic
# @keyword manip
print.reshape_df <- function(x, digits=getOption("digits"), ...) {
...
# Convert reshape matrix into a data frame
#
# @argument Reshape matrix
# @argument Argument required to match generic
# @argument Argument required to match generic
# @keyword manip
as.data.frame.reshape_matrix <- function(x, row.names, optional) {
	col.names <- attr(x, "r.col.names")
	row.names <- attr(x, "r.row.names")

	colnames(x) <- as.vector(apply(col.names, 1, function(x) paste(x, collapse=".")))
	r.df <- data.frame(row.names, unclass(x))

	class(r.df) <- c("reshape_df", "data.frame")
...
# Convert reshape data frame into a matrix
#
# @argument Reshape data frame
# @keyword manip
as.matrix.reshape_df <- function(x) {
	row.names <- attr(x, "r.row.names")
	...
	class(m) <- c("reshape_matrix", "matrix")
